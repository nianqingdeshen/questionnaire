...
engine = create_engine("mysql+pymysql://root:Zzb_2688_Gbkh@localhost/jxkh?charset=utf8")

# In[3]:

db = pymysql.connect(host="localhost", user="root",password="Zzb_2688_Gbkh", db="jxkh", port=3306, charset="utf8")
...
for j in range(0,len(resultlevel)):
    ...
    #print(deptlist[i])
        if(deptmemolist[i]==u"科室"):
            sql="SELECT COUNT(*) as num  FROM zc_ldbzkhinfo_"+code+" WHERE UserID IN (SELECT userID FROM userinformation WHERE  IsDB=1 AND deptid IN (SELECT deptid FROM  deptinfo WHERE DeptType=1)) and DeptID="+str(deptidlist[i])+" AND ZHKH="+            str(resultlevel[j])
        else:
...
df["qz_num"]=qz_num
xld_result=(df["xld_0"]/df["xld_num"]*1.0+df["xld_1"]/df["xld_num"]*0.8+df["xld_2"]/df["xld_num"]*0.6+df["xld_3"]/df["xld_num"]*0.0)*20
zc_result=(df["zc_0"]/df["zc_num"]*1.0+df["zc_1"]/df["zc_num"]*0.8+df["zc_2"]/df["zc_num"]*0.6+df["zc_3"]/df["zc_num"]*0.0)*20
bmm_result=(df["bmm_0"]/df["bmm_num"]*1.0+df["bmm_1"]/df["bmm_num"]*0.8+df["bmm_2"]/df["bmm_num"]*0.6+df["bmm_3"]/df["bmm_num"]*0.0)*40
qz_result=(df["qz_0"]/df["qz_num"]*1.0+df["qz_1"]/df["qz_num"]*0.8+df["qz_2"]/df["qz_num"]*0.6+df["qz_3"]/df["qz_num"]*0.0)*20
result=xld_result+zc_result+bmm_result+qz_result
df["zcbz_xld_result"]=xld_result
df["zcbz_zc_result"]=zc_result
df["zcbz_bmm_result"]=bmm_result
df["zcbz_qz_result"]=qz_result
df["zcbz_result"]=result
df2=pd.DataFrame()
df2=df[["DeptID","zcbz_xld_result","zcbz_zc_result","zcbz_bmm_result","zcbz_qz_result","zcbz_result"]]
...
zczz_xld_result=(df_zczz["xld_0"]/df_zczz["xld_num"]*1.0+df_zczz["xld_1"]/df_zczz["xld_num"]*0.8+df_zczz["xld_2"]/df_zczz["xld_num"]*0.6+df_zczz["xld_3"]/df_zczz["xld_num"]*0.0)*20
zczz_zc_result=(df_zczz["zc_0"]/df_zczz["zc_num"]*1.0+df_zczz["zc_1"]/df_zczz["zc_num"]*0.8+df_zczz["zc_2"]/df_zczz["zc_num"]*0.6+df_zczz["zc_3"]/df_zczz["zc_num"]*0.0)*20
zczz_bmm_result=(df_zczz["bmm_0"]/df_zczz["bmm_num"]*1.0+df_zczz["bmm_1"]/df_zczz["bmm_num"]*0.8+df_zczz["bmm_2"]/df_zczz["bmm_num"]*0.6+df_zczz["bmm_3"]/df_zczz["bmm_num"]*0.0)*40
resultzczz=zczz_xld_result+ zczz_zc_result+ zczz_bmm_result

df_zczz["zczz_xld_result"]=zczz_xld_result
df_zczz["zczz_zc_result"]=zczz_zc_result
df_zczz["zczz_bmm_result"]=zczz_bmm_result
...
df1_zczz=df_zczz[["Account","zczz_xld_result","zczz_zc_result","zczz_bmm_result","zczz_bzf" ,"zczz_fr"]]
...
zcfz_xld_result=(df_zcfz["xld_0"]/df_zcfz["xld_num"]*1.0+df_zcfz["xld_1"]/df_zcfz["xld_num"]*0.8+df_zcfz["xld_2"]/df_zcfz["xld_num"]*0.6+df_zcfz["xld_3"]/df_zcfz["xld_num"]*0.0)*20
zcfz_zc_result=(df_zcfz["zc_0"]/df_zcfz["zc_num"]*1.0+df_zcfz["zc_1"]/df_zcfz["zc_num"]*0.8+df_zcfz["zc_2"]/df_zcfz["zc_num"]*0.6+df_zcfz["zc_3"]/df_zcfz["zc_num"]*0.0)*20
zcfz_bmm_result=(df_zcfz["bmm_0"]/df_zcfz["bmm_num"]*1.0+df_zcfz["bmm_1"]/df_zcfz["bmm_num"]*0.8+df_zcfz["bmm_2"]/df_zcfz["bmm_num"]*0.6+df_zcfz["bmm_3"]/df_zcfz["bmm_num"]*0.0)*40
zcfz_zpf_result=(df_zcfz["zpf_0"]/df_zcfz["zpf_num"]*1.0+df_zcfz["zpf_1"]/df_zcfz["zpf_num"]*0.8+df_zcfz["zpf_2"]/df_zcfz["zpf_num"]*0.6+df_zcfz["zpf_3"]/df_zcfz["zpf_num"]*0.0)*10
resultzcfz=zcfz_xld_result+zcfz_zc_result+ zcfz_bmm_result+zcfz_zpf_result
df_zcfz["zcfz_xld_result"]=zcfz_xld_result
df_zcfz["zcfz_zc_result"]=zcfz_zc_result
df_zcfz["zcfz_bmm_result"]=zcfz_bmm_result
df_zcfz["zcfz_zpf_result"]=zcfz_zpf_result
...
df1_zcfz=df_zcfz[["Account", "zcfz_xld_result","zcfz_zc_result","zcfz_bmm_result","zcfz_zpf_result","zcfz_bzf","zcfz_fr"]]
