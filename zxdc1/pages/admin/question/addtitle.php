...
$query_rswj = "SELECT * FROM wjreg ORDER BY wjregID DESC";
$rswj = mysql_query($query_rswj, $connjxkh) or die(mysql_error());
$row_rswj = mysql_fetch_assoc($rswj);
   ...
   <div class="layui-form-item layui-form-text">
	...
	<div class="layui-inline">
 		<label class="layui-form-label">类型</label>
  		<div class="layui-input-inline ">
  			<select name="khtype" lay-filter="khtype">
        <option value=""></option>
        <option value="0">问卷调查</option>
        <option value="1" selected="">综合考评</option>       
      </select>
    	</div>
	</div>
	<div class="layui-inline">
 		<label class="layui-form-label">选择问卷</label>
  		<div class="layui-input-inline ">
  			<select name="wjregID" lay-filter="wjregID">
  			  <option value="">请选择问卷</option>
  			  <option value="0">综合考评</option>  			  
  			  <?php
do {  
?>
  			  <option value="<?php echo $row_rswj['wjregID']?>"><?php echo $row_rswj['wjName']?></option>
  			  <?php
} while ($row_rswj = mysql_fetch_assoc($rswj));
  $rows = mysql_num_rows($rswj);
  if($rows > 0) {
      mysql_data_seek($rswj, 0);
	  $row_rswj = mysql_fetch_assoc($rswj);
  }
?>       
      </select>
...
layui.use(['form', 'layedit', 'laydate','jquery'], function(){
 ...
 //创建时间
  ...
  form.on('submit(demo1)', function(data){
	  ...
	  var khtype=data.field.khtype;
	  var wjid=data.field.wjregID;
	 /* alert(recordname);
	  alert(description);
	  alert(welcome);
	  alert(end);
...
      data:"recordname="+recordname+"&description="+description+"&welcome="+welcome+"&end="+end+"&starttime="+starttime+"&endtime="+endtime+"&khtype="+khtype+"&wjid="+wjid,
      dataType:"json",
