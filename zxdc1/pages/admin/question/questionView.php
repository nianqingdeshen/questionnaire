...
<form >
<div class="layui-contains" >
<?php

mysql_select_db($database_connjxkh, $connjxkh);
/**按wjreg搜索数据库*/

$sql = "SELECT wjName FROM wjreg where wjregID =".$wjregID;
//echo $sql ;
$res = mysql_fetch_assoc(mysql_query($sql, $connjxkh));
echo "<div class='layui-fomr-item'><h1>&nbsp;&nbsp;&nbsp;&nbsp;".$res["wjName"]."</h1></div>"; // 调查名输出

if (!function_exists("Perparation")) {
    function Perparation($result,$connjxkh,$wjregID)
    {
        $cnt = 0;
        while ($res = mysql_fetch_assoc($result)) {//取出表study_sql中的所有结果集
			...
			//输出文本题答案
				if($type==2&&$parent_qid_item!=0) 
					echo "<input type='text' name='".$title."' value='".$question."'/>";
			//输出单选题答案
...
if ($count==0)
   echo "当前问卷还没有问题，请添新问题";
	...
	Perparation($result,$connjxkh,$wjregID);
