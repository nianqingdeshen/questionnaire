...
if ($_SESSION['MM_RoleID']==2 or $_SESSION['MM_RoleID']==3)
     if($_SESSION['MM_Memo']=='院部')
		$LevelID=4;
	 else
	   $LevelID=7; 	
else
	if($_SESSION['MM_Memo']=='科室')
		$LevelID=5;
	 else
	   $LevelID=6; 	
...
$sql="";
$sql11="";
$sql="INSERT INTO ".$tableName." (UserID,DeptID,LevelID, voteTime,ZHKH,bz) values";
//存入调查问卷
	for($x=0;$x<$length;$x++) {
		$CurrentTime=date('Y-m-d h:i:sa'); 
		if ($x==$length-1)
			$sql11 = $sql11."('$UserID','$DeptID[$x]','$LevelID','$CurrentTime','$name[$x]','$text[$x]')".";";
		else
			$sql11 = $sql11."('$UserID','$DeptID[$x]','$LevelID','$CurrentTime','$name[$x]','$text[$x]')".",";
