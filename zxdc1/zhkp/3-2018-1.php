		...
		<table class="layui-table" lay-size="sm" >
            <thead>
            	<tr>
                	...
                	<td colspan="1"  align = "center" >参加学习教育情况评价意见</td>
...
$sql = "SELECT * from userinformation where  not isBanned and (LevelID=2 OR LevelID=3 ) AND userid NOT IN (SELECT bpuserid FROM zc_ldcykhinfo_472739 WHERE userid=".$_SESSION['MM_UserID'].")  order by DeptID, orderID  LIMIT 0,30";
//$sql = "SELECT * from userinformation where  not isBanned and (LevelID=2 OR LevelID=3 ) AND userid NOT IN (SELECT bpuserid FROM zc_ldcykhinfo_472739 WHERE userid=".$_SESSION['MM_UserID'].") and DeptID != '".$result01['DeptID']."' order by DeptID, orderID  LIMIT 0,30";
...
while($row_rsUser = mysql_fetch_array($result))
{
	...
	for($i=0;$i<4;$i++){
		...
		<select name="manyi">
        <option value="0" selected = "selected">非常满意</option>
...
?>
		...
		<div class="layui-form-item layui-col-xs-offset6">
        	<input  class="layui-btn" type="button" onclick="test()" value="提交本页，进入下一页"/>
        </div>
	</form>
</body>
<script> 
...
function rec()
{
	var mymessage=confirm("确认提交进入下一页吗？");
	if(mymessage==true)
	{               
		document.write("确认");
	}
