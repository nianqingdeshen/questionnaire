...
$sql = "SELECT UserID,UserName,DeptName,deptinfo.DeptID FROM deptinfo, userinfo WHERE not userinfo.isBanned and userinfo.DeptID=deptinfo.DeptID AND Rank<4 LIMIT 0,30";
...
while($row_rsUser = mysql_fetch_array($result))
{
	...
	for($i=0;$i<4;$i++){
		$n1=rand(0,1000000);
		for($m=0;$m<4;$m++) 
			echo "<td><input name='".$n1."' checked='checked' type='radio' value='".$m."'></td>";
	}
}
