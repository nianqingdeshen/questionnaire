...
<table class="layui-table">
    ...
    <thead>
      <tr>
       <td colspan=7 align="center"><b>2019年度综合考核群众评价-中层领导班子评价意见</b></td>
      </tr> 
       <tr>
            <td>测评项目</td>
            <td>班子建设党的建设</td>
...
<table class="layui-table">
    <thead align="center">
     <tr>
       <td colspan=8 align="center"><b>2019年度综合考核群众评价-中层领导干部评价意见(<?php echo $rs['DeptName']?>)</td>
      </b></tr> 
       <tr align="center">
            <td rowspan="2" >姓名</td>
...
//查询该部门中中层领导干部的姓名和ID
$sql = "SELECT UserName,UserID FROM userinfo where not isBanned and DeptID=".$rs['DeptID']." and (LevelID=2 OR LevelID=3) order by orderID ";
...
//查询该部门的中层领导有多少位
$sql2 = "SELECT COUNT(LevelID) as number FROM userinfo WHERE not isBanned and DeptID=".$rs['DeptID']." AND LevelID=2 GROUP BY LevelID;";
$result2 = mysql_fetch_assoc(mysql_query($sql2, $connjxkh));
//while($row1 = mysql_fetch_array($result2))
$result_number= $result2['number'];	
if($result2['number']==0){
	$result_number=0;
}
echo "<input id='number1' name='number' type='hidden' value='".$result_number."'/>";

$sql3 = "SELECT COUNT(LevelID)as number FROM userinfo WHERE not isBanned and DeptID=".$rs['DeptID']." AND LevelID=3 GROUP BY LevelID;";
$result3 = mysql_fetch_assoc(mysql_query($sql3, $connjxkh));
$result_number1= $result3['number'];	
if($result3['number']==0){
	$result_number1=0;
}
echo "<input id='number2' name='number' type='hidden' value='".$result_number1."'/>";
