...
//查询用户等级
$mysql="select LevelID, IsDB, DeptMemo from userinformation where account='".$Account."'";
...
//中层正职领导菜单
if($rs['LevelID']==2  and $rs['DeptMemo']=='院部'){
...
if($rs['LevelID']==2  and $rs['DeptMemo']=='院部'  and $rs['IsDB']==1){
	$mymenu="select MenuID,MenuName,Menu_URL from menuinfo where MenuID in (1,2,3,4,5,6,8,10,11,12)";
	$result =mysql_query($mymenu, $connjxkh);
	$_SESSION['MM_Memo']='院部';
	$a=8;
}

if($rs['LevelID']==3  and $rs['DeptMemo']=='院部'  and $rs['IsDB']==1){
	$mymenu="select MenuID,MenuName,Menu_URL from menuinfo where MenuID in (1,2,3,4,5,8,10,11,12)";
	$result =mysql_query($mymenu, $connjxkh);
	$_SESSION['MM_Memo']='院部';
	$a=8;
}

if($rs['LevelID']==2 and $rs['DeptMemo']=='科室'){
	$mymenu="select MenuID,MenuName,Menu_URL from menuinfo where MenuID in (1,2,3,4,5,6,9,10,11,12)";
	$result =mysql_query($mymenu, $connjxkh);
	$_SESSION['MM_Memo']='科室';
	$a=9;
}

if($rs['LevelID']==3 and $rs['DeptMemo']=='科室'){
	$mymenu="select MenuID,MenuName,Menu_URL from menuinfo where MenuID in (1,2,3,4,5,9,10,11,12)";
	$result =mysql_query($mymenu, $connjxkh);
	$_SESSION['MM_Memo']='科室';
	$a=9;
}
...
<title><?php  echo iconv("GB2312","UTF-8",$zzmc.$xtmc); ?></title>
	...
	<body>
		...
		<div class="footer layui-bg-gray">
			<div class="copyright">Copyright ©2019 <?php echo iconv("GB2312","UTF-8",$zzmc.$ywbm);?>  </div>
		</div>
		<!-- 底部结束 -->
